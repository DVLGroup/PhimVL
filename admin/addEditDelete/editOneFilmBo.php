...
<html>
	<head>
		...
		$filmBoID = $_REQUEST['filmBoID'];
		require '../connect_db.php';
		$querySFB = "SELECT `film_bo_name`,`film_bo_name_vi`,`film_bo_sotap`,`film_bo_namsx`,B.film_cataloge_name,C.film_cataloge_name,
		D.film_cataloge_name,E.film_country_name,F.film_nhasx_name,`film_bo_content`,`film_bo_avatar`,`film_bo_cover`,`film_bo_daodien`,`film_bo_dienvien` 
		FROM film_bo A,film_cataloge B,film_cataloge C,film_cataloge D,film_country E,
		film_nhasx F WHERE A.film_bo_cataloge_id_first = B.film_cataloge_id and A.film_bo_cataloge_id_second = C.film_cataloge_id and 
		A.film_bo_cataloge_id_third = D.film_cataloge_id and A.film_bo_country_id = E.film_country_id and
		 A.film_bo_nhasx_id = F.film_nhasx_id and `film_bo_id` = $filmBoID";
		$resultSFB = mysql_query($querySFB);
		...
		while ($row = mysql_fetch_array($resultSFB)) {
			$filmBoName = $row['0'];
			$filmBoNameVi = $row['1'];
			$soTap = $row['2'];
			$namSX = $row['3'];
			...
			<div class="well">
				<form method="POST" action="../addEditDelete/editFilmBo.php" class="form-horizontal" id="management">
					...
					<div class="form-group">
						...
						<div class="col-md-8">
							<input type="text" name="filmBoTranName" class="form-control required" minlength="3" value="<?php echo($filmBoName); ?>" placeholder="Nhập Tên Thể Loại"  />
						</div>
					...
					<div class="form-group">
								...
								<div class="col-md-8">
								<input type="text" name="tenFilmBoVi" class="form-control required" value="<?php echo($filmBoNameVi); ?>" placeholder="Nhập Tên Tiếng Việt"  />
								</div>
								<div class="col-md-offset-2"></div>
							</div>
							<div class="form-group">
								<label class="control-label col-md-2">Số Tập</label>
								<div class="col-md-8">
								<input type="hidden" name="soTap" class="form-control digits required" value="<?php echo($soTap); ?>"/>
								<input disabled="disabled" type="text" class="form-control digits required" value="<?php echo($soTap); ?>" placeholder="Nhập Số Tập" />
								</div>
								<div class="col-md-offset-2"></div>
							</div>
							<div class="form-group">
						...
						<div class="col-md-offset-2 col-md-8">
							<div class="alert alert-warning">
								<strong>Cảnh Báo! </strong> Bạn Có Chắc Muốn Sửa Phim Có ID là: <strong><?php echo($filmBoID); ?></strong> Không?
							</div>
							<div class="btn-group">
								<a href="../index-admin.php?changePage=7" class="btn btn-info text-center">Không</a>
							</div>
